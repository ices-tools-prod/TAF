...
format (Patashnik 2003) originally designed for bibliographic information. The
benefit of using the BibTeX format for TAF metadata is that information about
all R packages is already available in this format, for example:
...
workflow author can specify the exact version for key software components that
...
FAO uses the `sraplus` package to analyze the status of global marine resources.
This package comes with over 100 package dependencies. The precise number of
dependencies changes over time, as underlying packages change their
dependencies. The `sraplus-deps` analysis is a simple workflow that describes
the dependencies, while the `WorkshopEffortShared` demonstrates the use of
`sraplus` for analyzing a small dataset.

Example        **`sraplus-deps`**
Analysis       Dependency analysis of the sraplus package
Repository     https://github.com/sofia-taf-dev/sraplus-deps
Methods        Base R package_dependencies function
External deps  2 GitHub repositories
Data sources   Online (software project on GitHub)
Results        2 tables, 1 plot
Notes          No local data files

Example        **`WorkshopEffortShared`**
Analysis       SOFIA analysis of the relative status of four marine populations
...
available online, for anyone to browse the data and results and/or rerun the
analysis.

Example        **`ofp-sam-yft-2023-diagnostic`**
Analysis       Assessment of yellowfin tuna in the Western and Central Pacific
Repository     https://github.com/PacificCommunity/ofp-sam-yft-2023-diagnostic
Methods        MULTIFAN-CL, C++ standalone executable
...
Data sources   Online (Johns Hopkins on GitHub) and 1 local file
Results        15 tables, 64 plots
Notes          Useful at the time to follow the global epidemic

Example        **`weather`**
Analysis       Compare climate data from selected cities around the world
Repository     https://github.com/arni-magnusson/weather
Methods        World map of UV, trigonometry for daylight calculations, etc.
External deps  3 CRAN packages
Data sources   Online (NASA Earth Observations website) and 15 local files
...
Methods        World map with flight routes, first test case for taf2html()
External deps  Many CRAN packages
Data sources   2 local files
Results        2 tables, 1 map
Notes          Produces webpage https://arni-magnusson.github.io/trip/
...
and deployment. *Linux Journal*, 239, 76-91.\
<https://dl.acm.org/doi/pdf/10.5555/2600239>

Patashnik, O. 2003. BibTEX yesterday, today, and tomorrow. *TUGboat*, 24,
25-30.\
...
Ushey, K. and Hadley Wickham. 2025. renv: Project Environments. R package
version 1.1.2.\
