...
regression example below but its functionality is covered in the <a
...
2. icesTAF loads all of TAF and adds additional features that are primarily
   used within ICES and may depend on external packages.
...
the corresponding `data`, `output`, and `report` folders. In contrast, the
`targets` package does not confine the user to a predetermined workflow design,
and tables containing data and results are generally accessed by the user as R
objects rather than files.

The `targets` package requires a higher level of R expertise than TAF, both for
the author to design a workflow using interconnected functions, and for a
...
not going to do it by itself. The work performed by TAF is what the author has
written in the scripts. Thanks to the general nature of the R language, R
scripts can do a wide variety of things, including running software written in
other languages than R.

There are essentially three paths to produce a TAF analysis: (1) start from
scratch using `taf.skeleton()`, (2) start from a template such as
`taf.example("linreg")` or another existing TAF analysis, or (3) add TAF scripts
to an already completed analysis, to strengthen its reproducibility and
reviewability.

Among the <a href="#appendix-online-examples">online examples</a>, one-off
analyses such as *corona*, *weather*, and *trip* are created from scratch, while
ICES and SOFIA analyses are often based on existing templates. The SPC analyses
of yellowfin and albacore tuna are examples where the model development and
model fitting is conducted independent of TAF using Bash shell scripts, but TAF
scripts are added afterwards to showcase the data and results of the analysis in
CSV format. This is especially helpful for the scientific committee reviewing
the analyses, as well as anyone else who would like to browse and use these
public data.
...
public once the analysis is ready for presentation. The skipjack growth analysis
is an example where only the scripts can be made public, as the data are
sensitive and cannot be shared. Sharing the scripts allows review and reuse of
the methods used in the analysis and the TAF repository serves as a technical
annex of the published report (Macdonald et al. 2025).
...
Macdonald, J. et al. (2025, in prep).
Skipjack growth analysis. Report to be presented to the Scientific Committee of
the Western and Central Pacific Fisheries Commission (WCPFC) in August 2025.
...
workflows covering areas outside of fisheries science.
...
The following examples demonstrate various uses of TAF covering areas outside of
fisheries science.
