...
Most of the metadata are of an informative nature for human consumption and not
subject to strict rules. This entry informs us that the data were prepared by
Mordecai Ezekiel and used in an analysis conducted in 1930. The only metadata
fields that are parsed by `taf.boot()` are the key `ezekiel.txt` and source =
`file`.
...
1. GitHub reference of the form `owner/repo[/subdir]@ref`, identifying a
   specific version of GitHub resource.

2. URL, identifying a file to download.

3. Boot script that should be run to fetch or produce files.

4. Relative path starting with `initial`, identifying the location of a file or
   directory provided by the user.

5. Special value `file` or `folder`, indicating a file or folder inside
   `boot/initial/data` or `boot/initial/software`.

The processing of metadata entries works in the same way for software and data,
and when software entries point to R packages they are installed in a local
library, specific for that workflow.

Something about renv and Docker.
