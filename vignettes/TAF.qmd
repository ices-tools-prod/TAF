...
not going to do it by itself. Generally speaking, the only work performed by TAF
is what the scripts contain. Luckily, R scripts can do a wide variety of things,
including running software written in other languages than R.

TAF is especially designed to showcase the final form of a scientific analysis.
There are essentially three paths to produce a TAF analysis: (1) starting the
from scratch using `taf.skeleton()`, (2) starting from a template such as
`taf.example("linreg")` or another existing TAF analysis, or (3) encapsulating
an already completed analysis, to strengthen reproducibility and reviewability.
...
public once the analysis is ready for presentation. Among the <a
href="#appendix-online-examples">online examples</a>, one-off analyses such as
*corona*, *weather*, and *trip* are created from scratch, while ICES and SOFIA
analyses are often based on existing templates.

The SPC analyses of yellowfin and albacore tuna are examples where the actual
analysis is conducted independent of TAF using Bash shell scripts, but TAF
scripts are added afterwards to extract and showcase the data and results of the
analysis in CSV format. This is especially helpful for the scientific committee
reviewing the assessments, as well as anyone else who would like to browse and
possibly use these public data.
