...
produced by the `data.R` and `output.R` scripts, the initial data files inside
`boot/initial/data` folder can be in any format. The recommendation in TAF is to
use the original format that the scientist received the data in, and use TAF
scripts to document the entire data import and preparation. Initial data files
can thus be a combination of text files (`.csv`, `.dat`, `.txt`), R data files
(`.rds`, `.RData`), zip archives (`.zip`, `.tar.gz`), spreadsheets (`.xlsx`,
`.ods`), and so on. Better still, boot scripts can be used to query databases or
fetch data from online resources that often have some level of data quality
