...
#' package.
#'
#' \code{\link{taf.boot}} is the procedure to install packages into a local
...
{
  ## If taf.library() is called from boot data script, the working
  ## directory is root/boot/data/scriptname; change to root temporarily
  if(basename(dirname(dirname(getwd()))) == taf.constants$boot)
  {
    owd <- setwd("../../.."); on.exit(setwd(owd))
  }

  if(!dir.exists(taf.lib.path()))
    stop("directory '", taf.lib.path() , "' not found")

  installed <- dir(taf.lib.path())
  if(missing(package))
    return(installed)

  package <- as.character(substitute(package))
  if(!(package %in% installed))
    stop("there is no package '", package, "' in ", taf.lib.path())

  supM <- if(messages) identity else suppressMessages
  supW <- if(warnings) identity else suppressWarnings
  supW(supM(library(package, lib.loc = taf.lib.path(), character.only = TRUE)))
}
