#' Clean Data
#'
#' Selectively remove data from the \verb{boot/data} folder if not listed
#' in \verb{DATA.bib}.
#'
...
#' }
#'
#' The \code{taf.boot} procedure cleans the \verb{boot/data} folder,
#' without requiring the user to run \code{clean.data}.
#'
#' @seealso
#' \code{\link{taf.boot}} calls \code{clean.data} as part of the default
#' boot procedure.
#'
...
clean.data <- function(quiet=FALSE, force=FALSE)
{
  folder <- taf.data.path()
