...
#' analysis. It looks for data files inside \verb{boot/initial/data} folder
#' and data scripts inside \verb{boot}.
#'
...
draft.data <- function(originator=NULL, year=format(Sys.time(),"%Y"),
                       ...
                       file="", append=FALSE,
                       data.files=dir(file.path(taf.constants$boot, "initial/data")),
                       data.scripts=dir(taf.constants$boot,pattern="\\.R$"))
{
  ...
  if(length(entries) == 0)
    stop("no data (", taf.constants$boot, "/initial/data/*) ",
         "or data scripts (", taf.constants$boot, "/*.R) found")
  if(is.null(source))
  ...
  if (identical(file, TRUE)) {
    file <- file.path(taf.constants$boot, "DATA.bib")
  }
