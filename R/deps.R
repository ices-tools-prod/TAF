...
#'
#' @details
#' The files scanned are those with the file extensions \code{.R}, \code{.Rmd},
#' \code{Rnw}, and \code{qmd}.
#'
#' @return
                 ...
                 list=FALSE)
{
  files <- if(dir.exists(path))
             dir(path, pattern="\\.(R|Rmd|Rnw|r|rmd|rnw|qmd)$", full.names=TRUE)
           else
             path
