                           ...
                           version=NULL, source=NULL, file="", append=FALSE)
{
  ...
  {
    targz <- download.github(package, taf.software.path())
    taf.install(file.path(taf.software.path(), targz))
  ...
  else if(dirname(package) == file.path(taf.constants$boot, "initial/software"))
  {
  ...
  if (identical(file, TRUE)) {
    file <- file.path(taf.constants$boot, "SOFTWARE.bib")
  }
