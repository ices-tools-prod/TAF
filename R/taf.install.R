...
#' without running the boot procedure, e.g. to avoid updating the
...
taf.install <- function(targz=NULL, lib=taf.lib.path(), quiet=FALSE)
{
  if(is.null(targz))
    targz <- dir(taf.software.path(), pattern="_[0-9a-f]{7}\\.tar\\.gz",
                 full.names=TRUE)
